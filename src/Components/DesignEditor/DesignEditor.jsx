        ...
        <Row>
            <Form.Group className="col-sm-4">
                ...
                <Form.Select name="pattern" onChange={onChange} value={state.pattern}>
                    { Constants.DESIGN_PATTERNS.map((item)=> <option value={item.value}>{item.name}</option>) }
                </Form.Select>
            </Form.Group>
            <Form.Group className="col-sm">
                <Form.Label>Direction</Form.Label>
                <Form.Select name="direction" onChange={onChange} value={state.direction}>
                    { Constants.DESIGN_DIRECTIONS.filter((item)=> item.pattern === state.pattern).map((item)=> <option value={item.value}>{item.name}</option>)  }
                </Form.Select>
            </Form.Group>
            <Form.Group className="col-sm">
